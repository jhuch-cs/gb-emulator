...
CPU::CPU(MMU& mmu, u8* bootRomLocation){
    this->mmu = mmu;
    this->bootRomLocation = bootRomLocation;
}
...
u8 CPU::exec(){
    //I think we need a read function to loop and go to memory where the program counter is to tell us what the next op code is
    //opCode = read(PC);
